...
export PHASE=1
DEPEND=""
PID=`sbatch --parsable $SLURMOPTSCPU1 $DEPEND ./postprocessPLM.sh`

export PHASE=2
DEPEND="--dependency=afterok:$PID"
PID=`sbatch --parsable $SLURMOPTSCPU1 --array=0-$(( $NF - 1 )) $DEPEND ./postprocessPLM.sh`
...
export PHASE=4
DEPEND="--dependency=afterok:$PID"
PID=`sbatch --parsable $SLURMOPTSGPU1 --array=0-$NBS $DEPEND ./postprocessPLM.sh`
