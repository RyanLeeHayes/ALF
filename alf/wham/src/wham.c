#include <stdio.h>
#include <math.h>
#include <stdlib.h> // CXX
#include <string.h> // CXX
...
//__device__ __host__ inline // CUDA
...
void atomic_logadd(double *p_lnA,double lnB)
{
  ...
  //  tmp_lnA=__longlong_as_double(atomicCAS((unsigned long long int*) p_lnA,__double_as_longlong(lnA),__double_as_longlong(lnC))); // CUDA
  //} while (lnA != tmp_lnA); // CUDA
  p_lnA[0]=lnC; // CXX
}
...
struct_data* readdata(int arg1, double arg2, int arg3, int arg4)
{
  ...
  //cudaMalloc(&(data->beta_d),data->NF*sizeof(double)); // CUDA
  ...
  // cudaMalloc(&(data->n_d),data->NF*sizeof(int)); // CUDA
  data->n_d=(int*) malloc(data->NF*sizeof(int)); // CXX
  data->ND=0;
  data->NDmax=0;
  ...
  // cudaMemcpy(data->n_d,data->n_h,data->NF*sizeof(int),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->n_d,(void*)data->n_h,data->NF*sizeof(int)); // CXX
  // cudaMalloc(&(data->D_d),data->NDmax*data->Ndim*sizeof(double)); // CUDA
  data->D_d=(double*) malloc(data->NDmax*data->Ndim*sizeof(double)); // CXX
  // cudaMemcpy(data->D_d,data->D_h,data->NDmax*data->Ndim*sizeof(double),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->D_d,(void*)data->D_h,data->NDmax*data->Ndim*sizeof(double)); // CXX

  // cudaMalloc(&(data->i_d),data->NDmax*sizeof(int)); // CUDA
  data->i_d=(int*) malloc(data->NDmax*sizeof(int)); // CXX
  // cudaMemcpy(data->i_d,data->i_h,data->NDmax*sizeof(int),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->i_d,(void*)data->i_h,data->NDmax*sizeof(int)); // CXX

  // cudaMalloc(&(data->lnw_d),data->NF*sizeof(double)); // CUDA
  data->lnw_d=(double*) malloc(data->NF*sizeof(double)); // CXX
  // cudaMemcpy(data->lnw_d,data->lnw_h,data->NF*sizeof(double),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->lnw_d,(void*)data->lnw_h,data->NF*sizeof(double)); // CXX

  data->lnDenom_h=(double*) malloc(data->NDmax*sizeof(double));
  // cudaMalloc(&(data->lnDenom_d),data->NDmax*sizeof(double)); // CUDA
  data->lnDenom_d=(double*) malloc(data->NDmax*sizeof(double)); // CXX
  ...
  */
  ...
  // cudaMalloc(&(data->f_d),data->NF*sizeof(double)); // CUDA
  data->f_d=(double*) malloc(data->NF*sizeof(double)); // CXX
  for (i=0; i<data->NF; i++) {
    data->f_h[i]=0.0;
  }
  // cudaMemcpy(data->f_d,data->f_h,data->NF*sizeof(double),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->f_d,(void*)data->f_h,data->NF*sizeof(double)); // CXX
  data->invf_h=(double*) malloc(data->NF*sizeof(double));
  // cudaMalloc(&(data->invf_d),data->NF*sizeof(double)); // CUDA
  data->invf_d=(double*) malloc(data->NF*sizeof(double)); // CXX

  fprintf(stderr,"Warning, DOS allocation is not sparse, requesting %d doubles\n",data->B[0].N*B_N);
  data->lnZ_h=(double*) malloc(B_N*sizeof(double));
  // cudaMalloc(&(data->lnZ_d),B_N*sizeof(double)); // CUDA
  data->lnZ_d=(double*) malloc(B_N*sizeof(double)); // CXX
  ...
  // cudaMalloc(&(data->dlnZ_d),B_N*sizeof(double)); // CUDA
  data->dlnZ_d=(double*) malloc(B_N*sizeof(double)); // CXX
  // cudaMalloc(&(data->dlnZ_dN),jN*B_N*sizeof(double)); // CUDA
  data->dlnZ_dN=(double*) malloc(jN*B_N*sizeof(double)); // CXX
  data->Gimp_h=(double*) malloc(B_N*sizeof(double));
  // cudaMalloc(&(data->Gimp_d),B_N*sizeof(double)); // CUDA
  data->Gimp_d=(double*) malloc(B_N*sizeof(double)); // CXX

  data->C_h=(double*) malloc(jN*sizeof(double));
  // cudaMalloc(&(data->C_d),jN*sizeof(double)); // CUDA
  data->C_d=(double*) malloc(jN*sizeof(double)); // CXX
  data->CV_h=(double*) malloc(jN*sizeof(double));
  // cudaMalloc(&(data->CV_d),jN*sizeof(double)); // CUDA
  data->CV_d=(double*) malloc(jN*sizeof(double)); // CXX
  data->CC_h=(double*) malloc(jN*jN*sizeof(double));
  // cudaMalloc(&(data->CC_d),jN*jN*sizeof(double)); // CUDA
  data->CC_d=(double*) malloc(jN*jN*sizeof(double)); // CXX
...
void resetlogdata(double *d,int N)
{
  int i;
  for (i=0; i<N; i++) {
    d[i]=-INFINITY;
...
void sumdenom(struct_data data)
{
  int t;
  int i;
  double lnwn;
...
void getf(struct_data data)
{
  ...
  for (t=0; t<data.ND; t++) {
    ...
    lnw=data.lnw_d[data.i_d[t]];
    E=data.D_d[t*data.Ndim+data.NL+1+i];
    invf=logadd(invf,lnw-beta*E-data.lnDenom_d[t]);
  }
...
void normf(double *f,double f_avg,int N)
{
  int i;
  for (i=0; i<N; i++) {
    f[i]-=f_avg;
...
void iteratedata(struct_data *data)
{
  ...
  for (itt=0; itt<mitt; itt++) {
    ...
    // sumdenom <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0]); // CUDA
    sumdenom(data[0]); // CXX
    // resetlogdata <<< (data->NF+BLOCK-1)/BLOCK, BLOCK >>> (data->invf_d,data->NF);
    // getf <<< data->NF, BLOCK >>> (data[0]); // CUDA
    getf(data[0]); // CXX

    // cudaMemcpy(data->invf_h,data->invf_d,data->NF*sizeof(double),cudaMemcpyDeviceToHost); // CUDA
    memcpy((void*)data->invf_h,(void*)data->invf_d,data->NF*sizeof(double)); // CXX
    f_sum=0;
    for (i=0; i<data->NF; i++) {
    ...
    fprintf(stdout,"\n");
    f_sum/=data->NF;
    // normf <<< (data->NF+BLOCK-1)/BLOCK,BLOCK >>> (data->f_d,f_sum,data->NF); // CUDA
    normf(data->f_d,f_sum,data->NF); // CXX
    if (escape_flag==1) {
      break;
    }
  }
...
void bin1(struct_data data,int i1)
{
  double q1;
  int t;
  int iB1;
...
void bin12(struct_data data,int i1,int i2)
{
  double q1,q2,q;
  int t;
  int iB12;
...
void bin2(struct_data data,int i1,int i2)
{
  double q1,q2;
  int t;
  int iB1,iB2;
...
double bin_all(struct_data *data,int *ptype,int i)
{
      ...
      if (s1==s2) {
        ...
        for (i1=data->block0[s1]; i1<data->block0[s1+1]; i1++) {
          if (i==0) {
            ...
            wnorm=1.0;
            // bin1 <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],i1); // CUDA
            bin1(data[0],i1); // CXX
        ...
        for (i1=data->block0[s1]; i1<data->block0[s1+1]; i1++) {
          for (i2=i1+1; i2<data->block0[s1+1]; i2++) {
            if (i==0) {
              ...
              // bin12 <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],i1,i2); // CUDA
              bin12(data[0],i1,i2); // CXX
          ...
          for (i1=data->block0[s1]; i1<data->block0[s1+1]; i1++) {
            for (i2=i1+1; i2<data->block0[s1+1]; i2++) {
              if (i==0) {
                ...
                wnorm=1.0/((data->Nsubs[s1]-1)/2.0);
                // bin2 <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],i1,i2); // CUDA
                bin2(data[0],i1,i2); // CXX
  ...
  // cudaMemcpy(data->Gimp_d,data->Gimp_h,B_N*sizeof(double),cudaMemcpyHostToDevice); // CUDA
  memcpy((void*)data->Gimp_d,(void*)data->Gimp_h,B_N*sizeof(double)); // CXX
...
void reactioncoord_phi(struct_data data,int i1)
{
  double q1;
  int t;
...
void reactioncoord_psi(struct_data data,int i1,int i2)
{
  double q1,q2;
  int t;
...
void reactioncoord_omega(struct_data data,int i1,int i2)
{
  double q1,q2;
  int t;
...
void reactioncoord_chi(struct_data data,int i1,int i2)
{
  double q1,q2;
  int t;
    ...
    for (s2=s1; s2<data->Nsites; s2++) {
      ...
      if (s1==s2) {
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          if (i==0) {
            // reactioncoord_phi <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j1); // CUDA
            reactioncoord_phi(data[0],j1); // CXX
          }
          i--;
        }
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          for (j2=j1+1; j2<data->block0[s1+1]; j2++) {
            if (i==0) {
              // reactioncoord_psi <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j1,j2); // CUDA
              reactioncoord_psi(data[0],j1,j2); // CXX
            }
            i--;
          }
        }
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          for (j2=data->block0[s1]; j2<data->block0[s1+1]; j2++) {
            if (j1 != j2) {
              if (i==0) {
                // reactioncoord_chi <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j1,j2); // CUDA
                reactioncoord_chi(data[0],j1,j2); // CXX
              }
              i--;
            }
          }
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          for (j2=data->block0[s1]; j2<data->block0[s1+1]; j2++) {
            if (j1 != j2) {
              if (i==0) {
                // reactioncoord_omega <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j1,j2); // CUDA
                reactioncoord_omega(data[0],j1,j2); // CXX
              }
              i--;
            }
          }
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          for (j2=data->block0[s2]; j2<data->block0[s2+1]; j2++) {
            ...
            i--;
            if (i==0) {
              // reactioncoord_chi <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j2,j1); // CUDA
              reactioncoord_chi(data[0],j2,j1); // CXX
            }
            i--;
          }
        }
        ...
        for (j1=data->block0[s1]; j1<data->block0[s1+1]; j1++) {
          for (j2=data->block0[s2]; j2<data->block0[s2+1]; j2++) {
            ...
            i--;
            if (i==0) {
              // reactioncoord_omega <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0],j2,j1); // CUDA
              reactioncoord_omega(data[0],j2,j1); // CXX
            }
            i--;
          }
        }
...
void get_lnZ(struct_data data,double beta)
{
  double E;
  double lnw;
  int t,i;
...
void get_dlnZ(struct_data data,int j1,double beta)
{
  double E,q;
  double lnw;
  int t,i;
...
void get_CC(struct_data data,int i,double beta,double wnorm,int ptype)
{
  int j1;
  int j2;
  int k;
  ...
  for (j1=0; j1<data.jN; j1++) {
  for (j2=0; j2<data.jN; j2++) {
  ...
  loc_CC=myCC;
    ...
    // jN=data->NL+5*data->NL*(data->NL-1)/2;
    // jN=gridDim.x;
    data.CC_d[data.jN*j1+j2]=loc_CC;
...
void getfofq(struct_data *data,double beta)
{
  ...
  for (i=0; i<iN; i++) {
    ...
    // resetlogdata <<< (B_N+BLOCK-1)/BLOCK, BLOCK >>> (data->lnZ_d,B_N); // CUDA
    resetlogdata(data->lnZ_d,B_N); // CXX
    // get_lnZ <<< (data->ND+(100*SBLOCK)-1)/(100*SBLOCK), 100 >>> (data[0],data->beta_t); // CUDA
    get_lnZ(data[0],data->beta_t); // CXX
    ...
    for (j1=0; j1<jN; j1++) {
      ...
      // resetlogdata <<< (B_N+BLOCK-1)/BLOCK, BLOCK >>> (&(data->dlnZ_dN[B_N*j1]),B_N); // CUDA
      resetlogdata(&(data->dlnZ_dN[B_N*j1]),B_N); // CXX
      // get_dlnZ <<< (data->ND+(100*SBLOCK)-1)/(100*SBLOCK), 100 >>> (data[0],j1,data->beta_t); // CUDA
      get_dlnZ(data[0],j1,data->beta_t); // CXX
      // cudaMemcpy(data->dlnZ_hN[j1],&(data->dlnZ_dN[B_N*j1]),B_N*sizeof(double),cudaMemcpyDeviceToHost); // CUDA
      memcpy((void*)data->dlnZ_hN[j1],(void*)&(data->dlnZ_dN[B_N*j1]),B_N*sizeof(double)); // CXX
    ...
    // get_CC <<< make_uint3(jN,jN,1), 100 >>> (data[0],i,data->beta_t,wnorm,ptype); // CUDA
    get_CC(data[0],i,data->beta_t,wnorm,ptype); // CXX
    // cudaMemcpy(data->CC_h,data->CC_d,jN*jN*sizeof(double),cudaMemcpyDeviceToHost); // CUDA
    memcpy((void*)data->CC_h,(void*)data->CC_d,jN*jN*sizeof(double)); // CXX
