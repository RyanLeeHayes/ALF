...
struct_data* readdata(int argc, char *argv[])
{
  ...
  data->weight_h=(double*) malloc(data->ND*sizeof(double));
  cudaMalloc(&(data->weight_d),data->ND*sizeof(double));
...
__global__
void getweight(struct_data data)
{
  int t,tmin,tmax;
  double lnw;
  double beta;
  ...
  for (t=tmin; t<tmax; t++) {
    // E=data.D_d[t][0];
    lnw=data.lnw_d[data.i_d[t]];
    E=data.D_d[t*data.Ndim];
    data.weight_d[t]=exp(lnw-beta*E-data.lnDenom_d[t]);
  }
}
...
void iteratedata(struct_data *data)
{
  ...
  sumdenom <<< (data->ND+BLOCK-1)/BLOCK, BLOCK >>> (data[0]);
  getweight <<< 1, BLOCK >>> (data[0]);
  cudaMemcpy(data->weight_h,data->weight_d,data->ND*sizeof(double),cudaMemcpyDeviceToHost);
  fp=fopen("Lambda/weight.dat","w");
  for (i=0; i<data->ND; i++) {
    fprintf(fp," %12.5g\n",data->weight_h[i]);
  }
  fclose(fp);

  fp=fopen("Lambda/Lambda.dat","w");
  for (i=0; i<data->ND; i++) {
    for (j=0; j<data->Nblocks; j++) {
      fprintf(fp," %8.6f",data->D_h[data->Ndim*i+1+j]);
    }
    fprintf(fp,"\n");
  }
  fclose(fp);
...
void justplotit(struct_data *data,double beta)
{
  int i,iN;
  int k;
