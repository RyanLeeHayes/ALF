...
exec(open('variablesprod.py').read())
# sysname=alf_info['name'] # Set by variablesprod.py
...
# MD production
dcd_file = pycharmm.CharmmFile(file_name='dcd/{}_{}{}.dcd'.format(sysname,'prod',itt), 
               file_unit=1,formatted=False,read_only=False)
res_file = pycharmm.CharmmFile(file_name='res/{}_{}{}.res'.format(sysname,'prod',itt), 
               file_unit=2,formatted=True,read_only=False)
lam_file = pycharmm.CharmmFile(file_name='res/{}_{}{}.lmd'.format(sysname,'prod',itt), 
...
if itt == 1:
    ...
    prv_rest = pycharmm.CharmmFile(file_name='res/{}_{}{}.res'.format(sysname,'prod',itt-1), 
...
# write.coor_pdb('dcd/{}_{}{}.pdb'.format(sysname,'prod',itt)) # write out final frame

#if openmm: pycharmm.lingo.charmm_script('omm clear')
#if blade: pycharmm.lingo.charmm_script('blade off')

# # collect lambda statistics
# proc_lam = pycharmm.CharmmFile(file_name='res/{}_{}{}.lmd'.format(sysname,'prod',itt), 
